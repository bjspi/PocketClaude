...
Änderungen BEIDE Stellen aktualisieren.

Die App schickt weiterhin den vollen Prompt-String im `system_prompt`-Feld
des Requests. Das Web-UI schickt nur `system_prompt_mode` (+ optional
`system_prompt` als Custom-Text) — der Server löst hier auf.
"""
...
def _substitute_placeholders(prompt: str) -> str:
    """Ersetzt Platzhalter im fertigen Prompt-String.

    - `{{currentDateTime}}` (Anthropic-Stil) → aktuelles UTC-Datum
    - `__CURRENT_DATETIME__` (App-Variante, weil `{{...}}` in Kotlin-Templates
      collidieren würde) → dasselbe Datum

    Wenn künftig weitere Platzhalter dazukommen, hier ergänzen.
    """
...
def resolve_system_prompt(
    ...
    """Löst einen `system_prompt_mode` zum tatsächlichen Prompt-String auf.

    - "STANDARD" → SYSTEM_PROMPT_STANDARD
    - "PERMISSIVE" → SYSTEM_PROMPT_PERMISSIVE
    - "ULTRA_LIBERAL" → SYSTEM_PROMPT_ULTRA_LIBERAL (mit {{currentDateTime}}-Substitution)
    - "CUSTOM" → custom (oder STANDARD wenn leer); auch hier Platzhalter ersetzen
    - None oder unbekannt → None (Caller fällt auf Server-Default zurück)
    """
    if not mode:
        return None
    m = mode.strip().upper()
    ...
    if m == "PERMISSIVE":
        return SYSTEM_PROMPT_PERMISSIVE
    if m == "ULTRA_LIBERAL":
        return _substitute_placeholders(SYSTEM_PROMPT_ULTRA_LIBERAL)
    if m == "CUSTOM":
        raw = (custom or "").strip() or SYSTEM_PROMPT_STANDARD
        return _substitute_placeholders(raw)
    return None
